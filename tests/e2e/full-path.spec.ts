...
async function openDemoEvent(page: Parameters<typeof login>[0]) {
  ...
  await page
    .getByRole("link", { name: /Midnight Frequency/i })
    .first()
    .click();
}
...
test.describe("Full path: discover → checkout → my tickets → redeem", () => {
  ...
  test("buyer can initiate Stripe checkout for a ticket", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /buy this ticket/i }),
    ).toBeVisible();
    await page.getByRole("button", { name: /continue to payment/i }).click();

    // Button redirects to Stripe Checkout; wait for Stripe domain or success URL.
    await page.waitForURL(/checkout\.stripe\.com|\/checkout\/success/, {
      timeout: 30_000,
    });
  });
...
test.describe("Marketplace: list → purchase", () => {
  ...
  test("seller can open list-for-resale dialog on a valid ticket", async ({
    page,
  }) => {
    await login(page, DEMO_BUYER);
    await page.goto("/tickets");
    const listButton = page
      .getByRole("button", { name: /list ticket/i })
      .first();
    if (await listButton.count()) {
      await listButton.click();
      await expect(
        page.getByRole("heading", { name: /list ticket/i }),
      ).toBeVisible();
