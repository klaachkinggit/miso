...
async function profileIdByEmail(
  ...
  const { data, error } = await client
    ...
    .single<{ id: string }>();
  if (error || !data)
    throw new Error(`Missing profile ${email}: ${error?.message}`);
  return data.id;
}
...
async function createCategory(
  ...
  const { data, error } = await client
    ...
    .single<{ id: string }>();
  if (error || !data)
    throw new Error(`Category seed failed: ${error?.message}`);
  return data.id;
}
...
async function createListingFixture(
  ...
  if (ticketError || !ticket)
    throw new Error(`Ticket seed failed: ${ticketError?.message}`);
  ...
  if (listingError || !listing)
    throw new Error(`Listing seed failed: ${listingError?.message}`);
  ...
  const { error: updateError } = await client
    ...
    .eq("id", ticket.id);
  if (updateError)
    throw new Error(`Ticket listing update failed: ${updateError.message}`);
...
test.describe("Organization public storefront", () => {
  ...
  test("scopes event pages and marketplace listings to one organization", async ({
    page,
  }) => {
    ...
    try {
      ...
      ids.tickets.push(
        listingA.ticketId,
        listingB.ticketId,
        inactiveListing.ticketId,
      );
      ids.listings.push(
        listingA.listingId,
        listingB.listingId,
        inactiveListing.listingId,
      );

      await page.goto(`/s/${orgASlug}`);
      await expect(
        page.getByRole("heading", { name: "Store Alpha" }),
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: /Alpha Scoped Night/i }),
      ).toBeVisible();
      await expect(page.getByText("Beta Scoped Night")).toHaveCount(0);

      if (
        (process.env.MISO_STOREFRONT_ROOT_DOMAINS ?? "")
          .split(",")
          .includes("localhost")
      ) {
        ...
        await page
          .getByRole("link", { name: /Alpha Scoped Night/i })
          .first()
          .click();
        await expect(page).toHaveURL(
          new RegExp(
            `^http://${orgASlug}\\.localhost:${port}/events/shared-drop`,
          ),
        );
        await expect(
          page.getByRole("heading", { name: "Alpha Scoped Night" }),
        ).toBeVisible();
      }
      ...
      await expect(
        page.getByRole("heading", { name: "Beta Scoped Night" }),
      ).toBeVisible();

      await page.goto(`/s/${orgASlug}/marketplace`);
      await expect(
        page.getByRole("heading", { name: "Store Alpha exchange" }),
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: /Alpha Scoped Night/i }),
      ).toBeVisible();
      ...
      await expect(
        page.getByRole("button", { name: /buy ticket/i }),
      ).toBeVisible();
      ...
      if (ids.listings.length)
        await client.from("resale_listings").delete().in("id", ids.listings);
      if (ids.tickets.length)
        await client.from("tickets").delete().in("id", ids.tickets);
      ...
      if (ids.events.length)
        await client.from("events").delete().in("id", ids.events);
      if (ids.organizations.length)
        await client.from("organizations").delete().in("id", ids.organizations);
    }
  });
});
