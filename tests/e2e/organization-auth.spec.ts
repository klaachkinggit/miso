...
async function ensureOrgController(client: SupabaseClient): Promise<string> {
  ...
  if (existing?.id) {
    await client.from("profiles").update({ role: "user" }).eq("id", existing.id);
    return existing.id;
  }
...
test.describe("Organization auth app boundary", () => {
  ...
  test("organization controller uses assigned gates but cannot use same-org buyer flows", async ({ page }) => {
    const client = sb();
    ...
    await client.from("event_controllers").upsert({ event_id: event!.id, user_id: userId });

    await login(page, ORG_CONTROLLER);

    const gateRes = await page.request.post("/api/controller/gates", {
      data: { event_id: event!.id, gate_name: "Org membership gate", ttl_hours: 1 },
    });
