...
async function seedListingFixture(
  ...
  sellerEmail: string,
): Promise<{ ticketId: string; listingId: string; categoryId: string; eventId: string }> {
...
async function teardown(client: SupabaseClient, ids: { eventId: string }) {
  await client.from("resale_listings").delete().eq("seller_user_id", "00000000-0000-0000-0000-000000000000");
...
test.describe("Cross-user authz", () => {
  ...
  test("non-owner cannot cancel another user's listing", async ({ page }) => {
    ...
    expect(String(body?.error ?? "").toLowerCase()).toContain("not listing owner");
  ...
  test("controller cannot use the marketplace (purchase, list, cancel)", async ({ page }) => {
