...
test.describe("Organizer category creation", () => {
  test.skip(!fullPathEnabled, "Set MISO_E2E_FULL=1 with a seeded organizer to run.");

  test.beforeEach(async ({ page }) => {
    await login(page, DEMO_ORGANIZER);
  });

  test("creates a standard category and seeds its tickets", async ({ page }) => {
  ...
  test("creates a club_table category (price doubles as min spending)", async ({ page }) => {
  ...
  test("creates a club_table with extra guests enabled", async ({ page }) => {
    ...
    await page.getByLabel("Base guests included").fill("4");
    await page
      .getByLabel(/allow extra guests beyond base capacity/i)
      .check();
