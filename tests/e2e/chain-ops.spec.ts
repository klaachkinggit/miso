...
test.describe("chain_ops invariants", () => {
  ...
  test("only one live mint op per purchase (partial unique index)", async () => {
    ...
    const first = await client
      ...
      .insert({
        ...baseRow,
        idempotency_key: `mint:${fx.purchaseId}:1`,
        status: "sent",
      })
      .select("id")
      .single();
    ...
    const second = await client
      ...
      .insert({
        ...baseRow,
        idempotency_key: `mint:${fx.purchaseId}:2`,
        status: "queued",
      })
      .select("id")
      .single();
    expect(
      second.error,
      "duplicate live mint op must be rejected",
    ).toBeTruthy();
    ...
    const third = await client
      ...
      .insert({
        ...baseRow,
        idempotency_key: `mint:${fx.purchaseId}:3`,
        attempt: 2,
        status: "queued",
      })
      .select("id")
    ...
    expect(
      third.error,
      "after errored attempt, new attempt is allowed",
    ).toBeFalsy();
  ...
  test("idempotency_key is globally unique across chain_ops", async () => {
    ...
    expect(
      second.error,
      "duplicate idempotency_key must violate unique index",
    ).toBeTruthy();
  ...
  test("op_type ↔ reference column check constraint", async () => {
    ...
    expect(
      bad.error,
      "mint without purchase_id must violate check constraint",
    ).toBeTruthy();
    ...
    expect(
      bad2.error,
      "transfer without listing_id must violate check constraint",
    ).toBeTruthy();
    ...
    expect(
      walletExport.error,
      "wallet_export without purchase/listing is valid",
    ).toBeFalsy();
  ...
  test("only one live wallet export op per ticket", async () => {
    ...
    const first = await client
      ...
      .insert({
        ...baseRow,
        idempotency_key: `wallet-export:${fx.ticketId}:1`,
        status: "sent",
      })
      .select("id")
      .single();
    ...
    const second = await client
      ...
      .insert({
        ...baseRow,
        idempotency_key: `wallet-export:${fx.ticketId}:2`,
        status: "queued",
      })
      .select("id")
      .single();
    expect(
      second.error,
      "duplicate live wallet export op must be rejected",
    ).toBeTruthy();
    ...
    const retry = await client
      ...
      .insert({
        ...baseRow,
        idempotency_key: `wallet-export:${fx.ticketId}:3`,
        attempt: 2,
        status: "queued",
      })
      .select("id")
    ...
    expect(
      retry.error,
      "after errored export attempt, new attempt is allowed",
    ).toBeFalsy();
