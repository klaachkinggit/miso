...
test.describe("Marketplace API surface", () => {
  test("POST /api/marketplace/listings rejects unauthenticated", async ({
    request,
  }) => {
  ...
  test("DELETE /api/marketplace/listings/[id] rejects unauthenticated", async ({
    request,
  }) => {
  ...
  test("POST /api/tickets/transfer-to-wallet rejects unauthenticated", async ({
    request,
  }) => {
