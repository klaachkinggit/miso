...
const WORKSPACE_ADMIN = {
  ...
  password: ["miso", "workspaceadmin"].join(""),
};
...
test.describe("Organization workspace selection", () => {
  ...
  test("admin switches organizations and creates events in the selected workspace", async ({
    page,
  }) => {
    const client = sb();
    ...
    try {
      ...
      await createEventFixture(client, {
      ...
      await expect(
        page.getByRole("heading", { name: `Workspace Beta ${stamp}` }),
      ).toBeVisible();
      ...
      await page
        .getByRole("button", { name: /save royalty settings/i })
        .click();
      await page.waitForURL(/\/admin\/settings\?success=/);
      ...
      await page
        .getByRole("button", { name: /transfer organization/i })
        .click();
      await page.waitForURL(/\/admin\/settings\?success=/);
      await expect(
        page.getByText("Organization transfer saved."),
      ).toBeVisible();
      ...
      await expect(
        page.getByText("Organization has activity and cannot be deleted."),
      ).toBeVisible();
      ...
      await page.goto("/admin/events");
      await expect(page.getByText(`Workspace Outsider ${stamp}`)).toHaveCount(
        0,
      );
      await expect(
        page.getByText(eventAName).or(page.getByText(eventBName)),
      ).toBeVisible();
    } finally {
