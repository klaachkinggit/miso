  ...
  return {
    ...
    transfers: {
      ...
      createReversal: async (
        ...
        args.recorder.reversalsCreated.push({
          transferId,
...
async function setUpFixtures(client: SupabaseClient): Promise<SeedFixtures> {
  ...
  await client
    .from("events")
    .update({ organizer_user_id: seller })
    .eq("id", ticket!.event_id);
  await client
    .from("ticket_categories")
...
async function tearDownFixtures(
  ...
  await client
    .from("ticket_categories")
    .update({
      price: f.originalCategoryPrice,
      currency: f.originalCategoryCurrency,
    })
    .eq("id", f.categoryId);
}
  ...
  test("onboarding → account.updated → checkout → succeeded webhook → paid → manual refund", async () => {
    ...
    try {
      ...
      const { data: post } = await client
        .from("stripe_seller_accounts")
        .select(
          "charges_enabled, payouts_enabled, details_submitted, seller_risk_status",
        )
        .eq("user_id", f.seller)
        .single<{
          charges_enabled: boolean;
      ...
      const { data: paymentAfterCheckout } = await client
        ...
        .select(
          "status, purchase_id, stripe_payment_intent_id, primary_seller_cents, marketplace_fee_cents",
        )
        .eq("id", paymentId)
        .single<{
          status: string;
      ...
      expect(paymentAfterCheckout!.stripe_payment_intent_id).toBe(
        paymentIntentId,
      );
      ...
      const { data: paymentAfterPaid } = await client
        .from("marketplace_payments")
        .select(
          "status, stripe_charge_id, succeeded_at, fulfilled_at, transferred_at, failure_reason",
        )
        .eq("id", paymentId)
        .single<{
          status: string;
      ...
      const { data: transfers } = await client
        .from("marketplace_transfers")
        .select(
          "status, stripe_transfer_id, recipient_role, amount_cents, recipient_user_id",
        )
        .eq("marketplace_payment_id", paymentId);
      ...
      expect(transferAfterReversal!.stripe_transfer_reversal_id).toMatch(
        /^trr_test_/,
      );
    } finally {
      resetStripeClientForTest();
