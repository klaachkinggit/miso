...
async function insertMarketplacePayment(
...
async function cleanup(
  ...
  if (ids.paymentId)
    await client.from("marketplace_payments").delete().eq("id", ids.paymentId);
  if (ids.purchaseId)
    await client.from("purchases").delete().eq("id", ids.purchaseId);
  if (ids.ticketId) {
    await client
      .from("tickets")
      .update({
