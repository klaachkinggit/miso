...
test.describe("Controller flow", () => {
  ...
  test("controller can open + poll + close a gate session", async ({
    page,
  }) => {
    await login(page, DEMO_CONTROLLER);
    ...
    const pollRes = await page.request.get(
      `/api/controller/gates/${session.id}`,
    );
  ...
  test("controller cannot poll a gate belonging to another controller", async ({
    page,
  }) => {
    const client = sb();
    ...
    if (!otherUserId) {
      const created = await client.auth.admin.createUser({
        ...
        password: ["miso", "controllerb"].join(""),
        email_confirm: true,
