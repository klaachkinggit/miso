...
import { ApiRouteError, DomainError, apiErrorResponse } from "@/lib/api/errors";
...
describe("getRequestOrigin", () => {
  ...
  it("uses canonical APP_URL for non-local requests and strips trailing slashes", () => {
    ...
    expect(getRequestOrigin(request("https://attacker.example/checkout"))).toBe(
      "https://miso.example.com",
    );
  });
  ...
  it("keeps localhost origins usable even when APP_URL is set", () => {
    ...
    expect(getRequestOrigin(request("http://localhost:3002/checkout"))).toBe(
      "http://localhost:3002",
    );
  });
  ...
  it("keeps trusted organization storefront origins for checkout redirects", () => {
    ...
    expect(
      getRequestOrigin(request("https://boilerroom.miso.com/api/checkout")),
    ).toBe("https://boilerroom.miso.com");
  });
});
...
describe("API error response safety", () => {
  it("returns explicit API route errors with their status", async () => {
    const response = apiErrorResponse(
      new ApiRouteError("Authentication required.", 401),
    );
  ...
  it("exposes domain errors but suppresses generic internals", async () => {
    const consoleError = vi
      .spyOn(console, "error")
      .mockImplementation(() => {});
    const domain = apiErrorResponse(
      new DomainError("Ticket category is not accepted at this gate"),
    );
    const generic = apiErrorResponse(new Error("database column leaked"), {
  ...
  it("suppresses generic error internals when no domain marker is present", async () => {
    ...
    const supabaseErr = {
      code: "23514",
      message:
        'new row for relation "ticket_categories" violates check constraint',
      details: "Failing row contains (...)",
      hint: null,
    };
    const response = apiErrorResponse(supabaseErr, {
      fallback: "Category could not be created.",
    });
