...
class Query {
  ...
  private applyUpsert(): Row | null {
    ...
    const existing = dbState.followers.find(
      (r) => r.organization_id === row.organization_id && r.user_id === row.user_id,
    );
    if (existing) {
  ...
  single() {
    if (this.upsertRow) return Promise.resolve({ data: this.applyUpsert(), error: null });
    const row = this.rows().find((r) => this.matches(r));
    return Promise.resolve({ data: row ?? null, error: row ? null : { message: "not found" } });
  }
  maybeSingle() {
    if (this.upsertRow) return Promise.resolve({ data: this.applyUpsert(), error: null });
    if (this.updatePatch) return Promise.resolve({ data: this.applyUpdateReturning(), error: null });
    const row = this.rows().find((r) => this.matches(r));
  ...
  then(resolve: (v: { data?: Row[]; error: null }) => void) {
    if (this.selectCols && !this.upsertRow && !this.updatePatch && !this.isDelete) {
      const list = this.rows().filter((r) => this.matches(r)).map((r) => this.withJoins(r));
...
describe("followOrganization", () => {
  it("is idempotent — re-following returns the same single row", async () => {
    ...
    const first = await followOrganization({ organizationId: "o1", userId: "u1" });
    const second = await followOrganization({ organizationId: "o1", userId: "u1" });

    expect(first.id).toBe(second.id);
    expect(dbState.followers.filter((r) => r.organization_id === "o1" && r.user_id === "u1")).toHaveLength(1);
  });

  it("resurrects an unsubscribed follow (clears unsubscribed_at)", async () => {
    const { followOrganization, unsubscribeByToken, isFollowing } = await import("@/lib/followers");
    const row = await followOrganization({ organizationId: "o1", userId: "u1" });
    await unsubscribeByToken(row.unsubscribe_token);
    expect(await isFollowing({ organizationId: "o1", userId: "u1" })).toBe(false);

    await followOrganization({ organizationId: "o1", userId: "u1" });
    expect(await isFollowing({ organizationId: "o1", userId: "u1" })).toBe(true);
  });
});
...
describe("ensureAutoFollow", () => {
  ...
  it("does not resurrect an explicit unsubscribe", async () => {
    const { followOrganization, unsubscribeByToken, ensureAutoFollow, isFollowing } = await import("@/lib/followers");
    const row = await followOrganization({ organizationId: "o1", userId: "u1" });
    await unsubscribeByToken(row.unsubscribe_token);
  ...
  it("never throws when the DB call rejects", async () => {
    ...
    await expect(ensureAutoFollow({ organizationId: "o1", userId: "u1" })).resolves.toBeUndefined();
...
describe("unsubscribeByToken", () => {
  it("flips unsubscribed_at and returns true once, false thereafter", async () => {
    const { followOrganization, unsubscribeByToken } = await import("@/lib/followers");
    const row = await followOrganization({ organizationId: "o1", userId: "u1" });
...
describe("listActiveFollowerEmails", () => {
  it("returns active followers with email and excludes unsubscribed", async () => {
    const { followOrganization, unsubscribeByToken, listActiveFollowerEmails } = await import("@/lib/followers");
    ...
    await followOrganization({ organizationId: "o1", userId: "u1" });
    const second = await followOrganization({ organizationId: "o1", userId: "u2" });
    ...
    expect(active).toEqual([
      { userId: "u1", email: "a@example.com", unsubscribeToken: expect.any(String) },
    ]);
  });
});
