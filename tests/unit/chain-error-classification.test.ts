...
describe("classifyChainError", () => {
  it("classes ChainOpRepairError as non_compensatable + repair_needed", () => {
    const err = new ChainOpRepairError("op_1", "0xtx", "post-mine write failed");
    expect(classifyChainError(err)).toEqual({ kind: "non_compensatable", state: "repair_needed" });
  });
  ...
  it("classes ChainOpInFlightError as non_compensatable + in_flight", () => {
    ...
    expect(classifyChainError(err)).toEqual({ kind: "non_compensatable", state: "in_flight" });
  });
  ...
  it("classes plain errors as compensatable", () => {
    expect(classifyChainError(new Error("buyer email missing"))).toEqual({ kind: "compensatable" });
  ...
  it("trusts error.name when cross-realm instanceof would miss", () => {
    ...
    expect(classifyChainError(lookalike)).toEqual({ kind: "non_compensatable", state: "repair_needed" });
  });
});
