...
describe("checkout attribution", () => {
  ...
  it("extracts a bounded source path from relative return paths", () => {
    expect(sourcePathFromReturnPath("/s/miso/events/drop?ticket=1")).toBe("/s/miso/events/drop");
  ...
  it("falls back to server path for non-storefront requests", () => {
    expect(checkoutTrackingOrigin(request("https://app.miso.com/api/checkout"), null)).toBe(
      "path:/api/checkout",
    );
  });
});
