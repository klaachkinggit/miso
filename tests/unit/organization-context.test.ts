...
describe("organization workspace context", () => {
  ...
  it("resolves organization storefront subdomains and ignores reserved hosts", () => {
    ...
    expect(storefrontSlugFromHost("boilerroom.shop.miso.com")).toBe("boilerroom");
  ...
  it("rewrites clean storefront host paths to the local fallback route", () => {
    ...
    expect(storefrontRewritePath("/events/drop", "boilerroom")).toBe("/s/boilerroom/events/drop");
    ...
    const request = new NextRequest("https://boilerroom.miso.com/events/drop?ticket=1", {
  ...
  it("keeps links fallback-local unless already on matching storefront host", () => {
    expect(
      storefrontPathForHost("boilerroom", "/s/boilerroom/events/drop", "/events/drop", "boilerroom.miso.com"),
    ).toBe("/events/drop");
    expect(
      storefrontPathForHost("boilerroom", "/s/boilerroom/events/drop", "/events/drop", "other.miso.com"),
    ).toBe("/s/boilerroom/events/drop");
    expect(organizationStorefrontOrigin("boilerroom")).toBe("https://boilerroom.miso.com");
  });
});
