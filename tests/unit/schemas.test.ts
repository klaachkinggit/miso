...
describe("CreateEventSchema – organizer_resale_royalty_bps", () => {
  ...
  it("coerces a string bps value", () => {
    const parsed = CreateEventSchema.parse({
      ...base,
      organizer_resale_royalty_bps: "250",
    });
  ...
  it("accepts the maximum allowed value of 5000", () => {
    const parsed = CreateEventSchema.parse({
      ...base,
      organizer_resale_royalty_bps: 5000,
    });
  ...
  it("rejects values above 5000", () => {
    expect(
      CreateEventSchema.safeParse({
        ...base,
        organizer_resale_royalty_bps: 5001,
      }).success,
    ).toBe(false);
  });
  ...
  it("rejects negative values", () => {
    expect(
      CreateEventSchema.safeParse({ ...base, organizer_resale_royalty_bps: -1 })
        .success,
    ).toBe(false);
  });
});
...
describe("TransferToWalletSchema", () => {
  it("accepts valid EVM destination address", () => {
    ...
    expect(parsed.destination_address).toBe(
