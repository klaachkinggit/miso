...
vi.mock("@/lib/organizations/public", () => ({
  organizationEventPath: (org: string, slug: string) => `/s/${org}/events/${slug}`,
}));
...
class Query {
  ...
  private rows(): Row[] {
    ...
    if (this.table === "organizations") return [...dbState.organizations.values()];
...
describe("joinWaitlist", () => {
  ...
  it("is idempotent — re-joining returns the same single row", async () => {
    ...
    expect(dbState.waitlists.filter((r) => r.event_id === "e1" && r.user_id === "u1")).toHaveLength(1);
  });
});
...
describe("notifyWaitlistHead", () => {
  beforeEach(() => {
    ...
    dbState.seq = 0;
    mocks.sendWaitlistAvailable.mockClear();
    dbState.events.set("e1", { id: "e1", name: "Sold Out Show", slug: "sold-out", organization_id: "o1" });
  ...
  it("picks the oldest un-notified entry, stamps fields, and emails it", async () => {
    ...
    expect(dbState.waitlists.find((r) => r.user_id === "second")!.notified_at).toBeNull();
  ...
  it("notifies exactly one head under two concurrent callers", async () => {
    ...
    const targets = mocks.sendWaitlistAvailable.mock.calls.map((c) => (c[0] as { userId: string }).userId);
