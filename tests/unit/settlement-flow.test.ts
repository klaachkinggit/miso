...
class PurchasesQuery {
  ...
  eq(_col: string, val: string) {
    ...
    if (this.updatePatch && this.filterId) {
      dbState.updates.push({ table: "purchases", patch: this.updatePatch, id: this.filterId });
...
class TicketsQuery {
  private filterId: string | null = null;
  select() { return this; }
  eq(_col: string, val: string) { this.filterId = val; return this; }
  maybeSingle() {
...
describe("settlePaidPurchase", () => {
  ...
  it("is idempotent when the ticket is already sold against this purchase", async () => {
    ...
    dbState.tickets.set("t1", { id: "t1", status: "sold", original_purchase_id: "p1" });
  ...
  it("refuses to re-settle a refunded purchase", async () => {
    ...
    await expect(settlePaidPurchase({ purchaseId: "p1" })).rejects.toThrow(/refunded/);
    expect(mocks.fulfillReservedTicket).not.toHaveBeenCalled();
  });
  ...
  it("flips to pending and throws FulfillmentPendingError on chain-in-flight", async () => {
    ...
    dbState.tickets.set("t1", { id: "t1", status: "reserved", original_purchase_id: null });
    ...
    const { settlePaidPurchase, FulfillmentPendingError } = await import("@/lib/payments/settlement");

    await expect(settlePaidPurchase({ purchaseId: "p1" })).rejects.toBeInstanceOf(FulfillmentPendingError);
    expect(dbState.updates).toEqual([
      expect.objectContaining({
        table: "purchases",
  ...
  it("settles failed purchase + rethrows on non-chain errors", async () => {
    ...
    dbState.tickets.set("t1", { id: "t1", status: "reserved", original_purchase_id: null });
    mocks.fulfillReservedTicket.mockRejectedValueOnce(new Error("buyer email missing"));
    const { settlePaidPurchase } = await import("@/lib/payments/settlement");

    await expect(settlePaidPurchase({ purchaseId: "p1" })).rejects.toThrow("buyer email missing");
