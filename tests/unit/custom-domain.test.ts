...
describe("normalizeCustomDomain / validateCustomDomain", () => {
  it("strips protocol, path, and port and lowercases", async () => {
    const { normalizeCustomDomain } =
      await import("@/lib/organizations/custom-domain");
    expect(normalizeCustomDomain("HTTPS://Tickets.Example.com/checkout")).toBe(
      "tickets.example.com",
    );
    expect(normalizeCustomDomain("http://shop.example.com:3000")).toBe(
      "shop.example.com",
    );
  ...
  it("accepts a valid FQDN", async () => {
    const { validateCustomDomain } =
      await import("@/lib/organizations/custom-domain");
    expect(validateCustomDomain("tickets.example.com")).toBe(
      "tickets.example.com",
    );
  });

  it("rejects empty / single-label / malformed hostnames", async () => {
    const { validateCustomDomain, CustomDomainError } =
      await import("@/lib/organizations/custom-domain");
    expect(() => validateCustomDomain("")).toThrow(CustomDomainError);
    expect(() => validateCustomDomain("localhost")).toThrow(CustomDomainError);
    expect(() => validateCustomDomain("not a domain")).toThrow(
      CustomDomainError,
    );
    expect(() => validateCustomDomain("-bad.example.com")).toThrow(
      CustomDomainError,
    );
  });
  ...
  it("rejects reserved miso storefront root domains", async () => {
    ...
    expect(() => validateCustomDomain("acme.miso.com")).toThrow(
      CustomDomainError,
    );
  });
});
...
describe("setOrganizationCustomDomain", () => {
  it("persists a normalized domain + token and clears verified_at", async () => {
    ...
    const { setOrganizationCustomDomain } =
      await import("@/lib/organizations/custom-domain");
...
describe("verifyOrganizationCustomDomain (local fallback)", () => {
  it("marks verified when DNS is unreachable and VERCEL is unset", async () => {
    ...
    const { verifyOrganizationCustomDomain } =
      await import("@/lib/organizations/custom-domain");
    const result = await verifyOrganizationCustomDomain({
      organizationId: "o1",
    });
  ...
  it("throws when no domain is set", async () => {
    ...
    const { verifyOrganizationCustomDomain, CustomDomainError } =
      await import("@/lib/organizations/custom-domain");
    await expect(
      verifyOrganizationCustomDomain({ organizationId: "o1" }),
...
describe("addDomainToVercelProject", () => {
  it("no-ops (no fetch) when env is unset", async () => {
    ...
    const { addDomainToVercelProject } =
      await import("@/lib/organizations/custom-domain");
