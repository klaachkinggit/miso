...
describe("validateAndPricePromo", () => {
  ...
  it("prices a percent discount with floor rounding", async () => {
    seedPromo({ id: "p1", code: "TEN", discount_kind: "percent", percent_off: 10 });
  ...
  it("matches the code case-insensitively", async () => {
    seedPromo({ id: "p1", code: "EarlyBird", discount_kind: "percent", percent_off: 20 });
    const { validateAndPricePromo } = await import("@/lib/promo");
    const result = await validateAndPricePromo({
      code: "earlybird",
      organizationId: "org-1",
  ...
  it("rejects when the clamped discount would leave nothing to settle", async () => {
    seedPromo({ id: "p1", code: "ALL", discount_kind: "percent", percent_off: 100 });
    const { validateAndPricePromo } = await import("@/lib/promo");
    await expect(
      validateAndPricePromo({ code: "ALL", organizationId: "org-1", grossCents: 1 }),
  ...
  it("rejects a code before its window opens", async () => {
    ...
    const { validateAndPricePromo } = await import("@/lib/promo");
    await expect(
      validateAndPricePromo({ code: "SOON", organizationId: "org-1", grossCents: 1000 }),
  ...
  it("rejects an expired code", async () => {
    ...
    const { validateAndPricePromo } = await import("@/lib/promo");
    await expect(
      validateAndPricePromo({ code: "OLD", organizationId: "org-1", grossCents: 1000 }),
  ...
  it("rejects when max_uses is exhausted", async () => {
    ...
    const { validateAndPricePromo } = await import("@/lib/promo");
    await expect(
      validateAndPricePromo({ code: "ONCE", organizationId: "org-1", grossCents: 1000 }),
  ...
  it("rejects an inactive code", async () => {
    ...
    const { validateAndPricePromo } = await import("@/lib/promo");
    await expect(
      validateAndPricePromo({ code: "OFF", organizationId: "org-1", grossCents: 1000 }),
  ...
  it("rejects a code belonging to a different org (cross-org isolation)", async () => {
    ...
    const { validateAndPricePromo } = await import("@/lib/promo");
    await expect(
      validateAndPricePromo({ code: "SAVE", organizationId: "org-2", grossCents: 1000 }),
