...
Historical ship plan for Miso SaaS v1. Companion docs: `docs/audits/2026-06-12-foundation-check.md` (architecture verdict), `docs/research/2026-06-12-competitive-analysis.md` (feature gap matrix + top-10), `docs/adr/` (decisions), `docs/CONTEXT.md` (domain glossary).

## Status — 2026-06-15 (v1 complete)

**All of P0 and P1 (P1.1–P1.8, including P1.6 AI) are shipped and merged to `development`**, plus an "Operator Editorial" landing redesign and rotating gate QR (PR #27). The acceptance checklist (§7) is fully ticked and ADRs 0001–0004 record the hard-to-reverse decisions. **Remaining work is P2 stretch only** (§3) — nothing in P0/P1 is outstanding. Treat the sections below as historical spec + the remaining P2 backlog; for current architecture and domain terms see `docs/CONTEXT.md`, and for the latest session state see `docs/handoffs/`.
...
- Settlement backstop: the historical target was a Vercel cron every 10 min, but the current Hobby-compatible schedule is daily (`0 2 * * *`) until the project moves to Vercel Pro; Stripe event reconciliation is daily (`0 3 * * *`).
...
- Rotating QR for gate — shipped in PR #27.
...
- Workflows: sequential pipeline stages for anything touching `src/lib/stripe-marketplace/`; parallel builders only on disjoint file sets, followed by an integration verify stage.
- Session-limit resilience: inventory completed task outputs before re-running anything; keep the working tree committed before launching any workflow so a failed builder can't strand uncommitted work.
...
- [x] Production deploy: Vercel + Supabase prod + marketplace Stripe webhook endpoint + cron routes (code + runbook ready; requires prod credentials — see `docs/ci-and-deploy.md`)
...
**Beyond plan:** "Operator Editorial" landing redesign; rotating gate QR (PR #27); Codex harness cleanup (PR #28 follow-up); `.env` protection hook hardened.
