...
- **Embeddings**: **OpenAI `text-embedding-3-small` (1536-dim)** through `@ai-sdk/openai`. Anthropic has **no embeddings API**, so RAG cannot be single-vendor. OpenAI is introduced *solely* for embeddings.
...
- The **embedding model and its 1536 dimensions are baked into the schema** (column type + HNSW index). Changing the model or dimensionality means a migration *and* re-embedding all stored content.
