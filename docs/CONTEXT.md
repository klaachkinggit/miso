...
| **Account Balance** | Internal demo credit linked to a buyer/admin profile for one currency, stored as the current available amount and used for all demo checkout flows, resale settlement, and refunds. |
...
- Holder (1) → Account Balances (N) — at most one per supported currency when demo credit exists.
...
- **"Admin top-up"**: admins grant Account Balance credit from the admin area by choosing a holder, currency, and amount.
