...
| `THIRDWEB_BACKEND_SMART_WALLET_ADDRESS` | Vercel (Prod + Preview, optional) | Preferred `from` wallet for contract writes. If unset, code fetches the project's Server Wallet smart account from Thirdweb. Not secret, but environment-specific.        |
...
| `STRIPE_SECRET_KEY`                     | Vercel (Prod + Preview)           | Stripe dashboard → API keys → restricted key (`rk_...`). **Server-only.** Current marketplace env validation rejects `sk_...`. Use live key in Prod, test key in Preview. |
...
| `MISO_RESALE_PLATFORM_FEE_PERCENT`      | Vercel (Prod + Preview, optional) | Legacy public resale pricing fallback (default `5`). Not secret.                                                                                                          |
| `MISO_RESALE_PLATFORM_FEE_FIXED`        | Vercel (Prod + Preview, optional) | Legacy public resale pricing fallback (default `0`). Not secret.                                                                                                          |
