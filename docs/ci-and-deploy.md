...
| --- | --- | --- |
| `/api/cron/settlement` | `*/10 * * * *` (every 10 min) | Re-drives marketplace payments whose settlement lease expired (`reDriveStuckPayments`). |
...
| `SUPABASE_SERVICE_ROLE_KEY` | Vercel (Prod + Preview) | Supabase dashboard → API → service_role key. **Server-only, bypasses RLS.** Never `NEXT_PUBLIC_*`. |
...
| `STRIPE_WEBHOOK_SECRET` | Vercel (Prod + Preview) | Stripe dashboard → Webhooks → signing secret (`whsec_...`) for the core payments webhook endpoint. **Server-only.** |
...
`SENTRY_ORG`, `SENTRY_PROJECT`) are added by the observability work — see §6. They are
not yet in `.env.example`; add them to Vercel when that lands. Email (Resend) is **not
integrated in the codebase yet**; add `RESEND_API_KEY` (server-only) only once the
sending code exists — do not add a dangling secret.
...
3. [ ] All env vars set per the matrix, secrets server-only, `CRON_SECRET` set (§4).
