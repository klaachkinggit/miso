...
export function WaitlistAvailable(props: WaitlistAvailableProps) {
  return (
    ...
    >
      <Paragraph>
        Good news — a ticket for <strong>{props.eventName}</strong> may be available again, and you
        are next on the waitlist.
      </Paragraph>
      <Section style={{ margin: "0 0 18px" }}>
        <DetailRow label="Event" value={props.eventName} />
      </Section>
      <Paragraph>
        Tickets are first come, first served, so head over now to grab it before someone else does.
