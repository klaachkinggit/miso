...
function isReservedRootDomain(domain: string): boolean {
  return storefrontRootDomains().some((root) => domain === root || domain.endsWith(`.${root}`));
}
...
export function validateCustomDomain(value: string | null | undefined): string {
  ...
  if (!HOSTNAME_RE.test(domain)) throw new CustomDomainError("Enter a valid domain (e.g. tickets.example.com).");
  if (isReservedRootDomain(domain)) throw new CustomDomainError("That domain is reserved.");
  return domain;
}
...
export async function setOrganizationCustomDomain(params: {
  ...
  if (error) {
    ...
    if (error.code === "23505") throw new CustomDomainError("That domain is already in use.");
    throw new CustomDomainError(error.message);
  }

  return { domain, token, txtRecordName: `${CUSTOM_DOMAIN_TXT_PREFIX}.${domain}` };
}

async function dnsTxtMatchesToken(domain: string, token: string): Promise<boolean | null> {
  ...
  try {
    ...
    const records = await dns.resolveTxt(`${CUSTOM_DOMAIN_TXT_PREFIX}.${domain}`);
...
export async function verifyOrganizationCustomDomain(params: {
  ...
  const { data, error } = await sb
    ...
    .eq("id", params.organizationId)
    .maybeSingle<{ custom_domain: string | null; custom_domain_verification_token: string | null }>();
  ...
  const dnsResult = await dnsTxtMatchesToken(data.custom_domain, data.custom_domain_verification_token);
...
export async function resolveOrgSlugByCustomDomain(host: string | null | undefined): Promise<string | null> {
