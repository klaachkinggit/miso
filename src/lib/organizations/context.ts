...
export async function getAdminOrganizations(
  ...
  return organizationIds
    .map((id) => byId.get(id))
    .filter(
      (organization): organization is OrganizationOption => !!organization,
    );
}

export async function getActiveAdminOrganization(
...
export async function requireActiveAdminOrganization(
  profile: Pick<Profile, "id" | "role">,
): Promise<{
  organizations: OrganizationOption[];
...
export async function setActiveAdminOrganization(
  userId: string,
