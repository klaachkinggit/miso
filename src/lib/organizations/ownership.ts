...
export async function transferOrganizationOwnership(params: {
  ...
  const sb = createServiceClient();
  const recipientUserId = await findOrInvitePlatformAccount(params.recipientEmail);
...
export async function deleteEmptyOrganization(params: {
  ...
  const activity = await countOrganizationDeletionActivity(params.organization.id);
