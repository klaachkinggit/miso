...
import { getPublishedEventById, type PublicEventCategory } from "@/lib/events/public";
...
export async function getEmbedCategory(categoryId: string): Promise<EmbedCategory | null> {
  ...
  return {
    category: { ...category, remaining: Math.max(0, category.supply - category.sold_count) },
    event,
  };
}
