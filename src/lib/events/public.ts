...
async function activeOrganizationIdsForEvents(
  events: EventRow[],
): Promise<Set<string>> {
  ...
  if (error)
    throw new Error(`Organization status lookup failed: ${error.message}`);
  return new Set((data ?? []).map((organization) => organization.id));
}

async function filterActiveOrganizationEvents(
  events: EventRow[],
): Promise<EventRow[]> {
  const activeIds = await activeOrganizationIdsForEvents(events);
  return events.filter(
    (event) => !event.organization_id || activeIds.has(event.organization_id),
  );
}
...
export async function listPublishedEvents(params: {
  ...
  if (priceRange && events.length) {
    ...
    if (categoryError)
      throw new Error(`Event price discovery failed: ${categoryError.message}`);
  ...
  if (params.discovery.sort === "popular" && events.length) {
    ...
    if (popularityError)
      throw new Error(
        `Event popularity discovery failed: ${popularityError.message}`,
      );
    ...
    events = [...events].sort(
      (a, b) => (popByEvent.get(b.id) ?? 0) - (popByEvent.get(a.id) ?? 0),
    );
  }
...
export async function getPublishedEventByOrganizationSlug(params: {
  ...
  const { data, error } = await sb
    ...
    .maybeSingle<EventRow>();
  if (error)
    throw new Error(`Storefront event lookup failed: ${error.message}`);
  return data ?? null;
}

export async function listPublicEventCategories(
  eventId: string,
): Promise<PublicEventCategory[]> {
  const sb = createServiceClient();
  const { data, error } = await sb
    .from("ticket_categories")
    .select("*")
  ...
  if (error)
    throw new Error(`Event categories lookup failed: ${error.message}`);
