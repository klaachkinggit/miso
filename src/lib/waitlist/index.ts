...
export async function eventHasAvailability(params: {
  ...
  const { data, error } = await sb
    ...
    .eq("event_id", params.eventId)
    .returns<Array<{ supply: number; sold_count: number; sales_enabled: boolean }>>();
  if (error) throw error;
  return (data ?? []).some(
    (category) => category.sales_enabled && category.supply - category.sold_count > 0,
  );
}
    ...
    if (event?.organization_id) {
      ...
      if (organization) {
        if (params.source === "resale" && params.listingId) {
          eventUrl = organizationMarketplaceListingPath(organization.slug, params.listingId);
        } else if (event.slug) {
