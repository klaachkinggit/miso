...
function toSellable(params: {
  ...
  activeOrganizationsById?: Map<
    string,
    { resale_royalty_enabled: boolean; resale_royalty_bps: number }
  >;
  ...
  const listingOrganizationId =
    listing.organization_id ?? event.organization_id;
  const organization = listingOrganizationId
    ? activeOrganizationsById?.get(listingOrganizationId)
    : undefined;
  if (listingOrganizationId && activeOrganizationsById && !organization)
    return null;
...
async function activeOrganizationsById(
  ...
  Map<string, { resale_royalty_enabled: boolean; resale_royalty_bps: number }>
> {
  ...
  if (error)
    throw new Error(
      `Marketplace organization status lookup failed: ${error.message}`,
    );
  return new Map(
...
export async function listSellableResaleListings(
  params: {
    organizationId?: string;
    limit?: number;
  } = {},
  ...
  const { data: listings, error: listingError } =
    await query.returns<ResaleListing[]>();
  if (listingError)
    throw new Error(
      `Marketplace listings lookup failed: ${listingError.message}`,
    );

  const ticketIds = [
    ...new Set((listings ?? []).map((listing) => listing.ticket_id)),
  ];
  ...
  if (ticketError)
    throw new Error(
      `Marketplace tickets lookup failed: ${ticketError.message}`,
    );
  ...
  if (eventError)
    throw new Error(`Marketplace events lookup failed: ${eventError.message}`);
  if (categoryError)
    throw new Error(
      `Marketplace categories lookup failed: ${categoryError.message}`,
    );
    ...
    .map((listing) =>
      toSellable({
        ...
        category: categoryById.get(
          ticketById.get(listing.ticket_id)?.category_id ?? "",
        ),
...
export async function getSellableResaleListing(params: {
  ...
  if (
    ...
    listing.organization_id !== params.organizationId
  )
    return null;
  ...
  if (categoryError)
    throw new Error(
      `Marketplace category lookup failed: ${categoryError.message}`,
    );
  const activeOrganizations = await activeOrganizationsById([
