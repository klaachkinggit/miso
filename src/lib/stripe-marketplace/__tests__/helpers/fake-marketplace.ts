...
function matches(row: Row, eqs: FilterEq[], neqs: FilterEq[], ins: FilterEq[]): boolean {
...
class TableQuery {
  ...
  constructor(private readonly store: Map<string, Row>, private readonly table: string) {}
  ...
  private commitInsert(): Row[] {
    ...
    for (const input of list) {
      const row: Row = { id: crypto.randomUUID(), created_at: new Date().toISOString(), ...input };
  ...
  private resolveRows(): { rows: Row[]; error: unknown } {
    if (this.mode === "insert") return { rows: this.commitInsert(), error: null };
    if (this.mode === "update") return { rows: this.commitUpdate(), error: null };
    let rows = this.rows();
...
class FakeDb {
  ...
  reserveNextTicket(buyerUserId: string): Row {
    const free = [...this.tickets.values()].find((t) => t.status === "available");
    const ticket =
      free ??
      (() => {
...
class FakeStripe {
  ...
  paymentIntents = {
    create: vi.fn(async (params: { amount: number; metadata: Record<string, string> }) => {
