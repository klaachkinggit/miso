...
export async function assertEventPublishable(params: {
  ...
  const paidCategories = categories.filter(
    (category) => Number(category.price) > 0,
  );
  const paidMad = paidCategories.find(
    (category) => category.currency === "MAD",
  );
  if (paidMad) {
    throw new Error(
      "Paid MAD inventory cannot be published yet. MAD payment is not supported yet; use EUR for paid ticket sales.",
    );
  }
  ...
  const paidEur = paidCategories.find(
    (category) => category.currency === "EUR",
  );
  if (paidEur) {
    if (!event.organizer_user_id) {
      throw new Error(
        "Assign an organizer before publishing paid EUR inventory.",
      );
    }
