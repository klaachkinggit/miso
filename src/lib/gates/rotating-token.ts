...
function bucketToken(gateSessionId: string, bucket: number, key: string): string {
...
export function currentGateToken(gateSessionId: string, nowMs: number = Date.now()): string | null {
  const key = secret();
  if (!key) return null;
