...
export async function loadOrganizerOverview(params: {
  ...
  const scopedToOrganization =
    params.organizationId && organizationIds.includes(params.organizationId);
  ...
  const [eventsRes, categoriesRes, purchasesRes, redemptionsRes] =
    await Promise.all([
      eventsQuery,
      sb
        .from("ticket_categories")
        .select("event_id, supply, sold_count, currency"),
      sb
        .from("purchases")
  ...
  for (const row of categories) {
    ...
    soldByEvent.set(
      row.event_id,
      (soldByEvent.get(row.event_id) ?? 0) + row.sold_count,
    );
    supplyByEvent.set(
      row.event_id,
      (supplyByEvent.get(row.event_id) ?? 0) + row.supply,
    );
  ...
  for (const purchase of purchases) {
    ...
    revenueByEvent.set(
      purchase.event_id,
      (revenueByEvent.get(purchase.event_id) ?? 0) + Number(purchase.amount),
    );
  }
  ...
  for (const ticket of redemptions) {
    ...
    redeemedByEvent.set(
      ticket.event_id,
      (redeemedByEvent.get(ticket.event_id) ?? 0) + 1,
    );
  }
  ...
  const totals: OrganizerTotals = {
    ...
    tickets_redeemed: Array.from(redeemedByEvent.values()).reduce(
      (a, b) => a + b,
      0,
    ),
    total_capacity: Array.from(supplyByEvent.values()).reduce(
      (a, b) => a + b,
      0,
    ),
    revenue_paid: Array.from(revenueByEvent.values()).reduce(
      (a, b) => a + b,
      0,
    ),
    currency: totalsCurrency,
