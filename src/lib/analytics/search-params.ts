...
function parsePreset(raw: string | undefined): AnalyticsRangePreset {
  if (raw && (KNOWN_PRESETS as string[]).includes(raw))
    return raw as AnalyticsRangePreset;
  return "30d";
}
...
export function parseAnalyticsSearchParams(
  ...
  const compareRaw =
    typeof searchParams.compare === "string" ? searchParams.compare : undefined;
  const compare: "prior" | "none" = compareRaw === "none" ? "none" : "prior";
  const prior = compare === "prior" ? computePriorRange(range) : null;

  const filters: AnalyticsFilters = {
    eventIds: parseCsv(
      typeof searchParams.events === "string" ? searchParams.events : undefined,
    ),
    salesChannels: parseCsv(
      typeof searchParams.channels === "string"
