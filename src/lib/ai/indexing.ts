...
import { embedTexts, embeddingsEnabled, toVectorLiteral } from "@/lib/ai/embeddings";
...
function categoryChunk(category: CategoryRow, eventName: string): Chunk {
  ...
  return {
    ...
    content: lines.join("\n"),
    metadata: { name: category.name, price: category.price, currency: category.currency },
  };
}
...
export async function reindexOrganization(
  ...
  const { data: events } = await supabase
    .from("events")
    .select("id, name, description, date, venue_name, city, genre, vibe, artists")
