...
export function ResaleListingDetail({
  ...
  return (
    <div className="container grid gap-8 py-10 lg:grid-cols-[minmax(0,1fr)_400px]">
      <div className="space-y-6">
        <div className="relative aspect-[16/9] overflow-hidden rounded-md border border-hairline bg-secondary">
          {(() => {
        ...
        <div>
          <p className="eyebrow-signal mb-3">Verified member listing</p>
          <h1 className="display text-5xl text-foreground md:text-7xl">
            {item.event.name}
          </h1>
          <div className="mt-5 grid gap-2 font-mono text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
        ...
        <Card className="rounded-md border-hairline bg-card">
          <CardHeader>
          ...
          <CardContent className="space-y-3 text-sm leading-6 text-muted-foreground">
            <p>
              Purchase settles via Stripe. MISO handles the verified ticket
              transfer after payment, keeping resale inside the official
              anti-scalping flow.
            </p>
            <Separator />
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="text-foreground">
                      {item.ticket.extra_guests_count} added
                    </p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">
                      Remaining due at venue
                    </p>
                    <p className="text-foreground">
                  ...
                  <div>
                    ...
                    <p className="flex items-center gap-2 text-foreground">
                      <Circle
                        className="h-4 w-4"
                        fill={
                          item.ticket.color_hex_snapshot ??
      ...
      <aside className="space-y-4">
        <Card className="sticky top-24 rounded-md border-hairline bg-card">
          <CardContent className="space-y-4 p-5">
            <div>
              <p className="eyebrow">You pay today</p>
              <p className="display mt-2 text-4xl text-foreground">
              ...
              <div className="mt-3 grid gap-1 text-sm text-muted-foreground">
                ...
                <div className="flex justify-between gap-3">
                  ...
                  <span>
                    {formatPrice(item.platformFee, item.listing.currency)}
                  </span>
                </div>
                {item.royaltyAmount > 0 ? (
                  <div className="flex justify-between gap-3">
                    ...
                    <span>
                      {formatPrice(item.royaltyAmount, item.listing.currency)}
                    </span>
                  </div>
                ) : null}
                <div className="flex justify-between gap-3">
                  ...
                  <span>
                    {formatPrice(item.stripeFee, item.listing.currency)}
                  </span>
                </div>
              </div>
              {item.ticket.min_spending_remaining != null ? (
                <p className="mt-3 rounded-md border border-hairline-strong bg-ink-soft p-3 text-xs text-muted-foreground">
                  As-is resale: you inherit the table headcount and the
                  remaining minimum spending balance due at the venue (
                  {formatPrice(
                    item.ticket.min_spending_remaining,
                    item.category.currency,
                  )}
