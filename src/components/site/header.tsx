...
export async function Header() {
  ...
  const [adminOrganizationIds, memberOrganizationIds] = profile
    ? await Promise.all([
        getAdminOrganizationIds(profile.id),
        getMemberOrganizationIds(profile.id),
      ])
    : [[], []];
  const controllerOnly = profile?.role === "controller";
  const canUseWorkspace =
    !!profile &&
    (canUseOrganizerWorkspace(profile) || adminOrganizationIds.length > 0);
  const canUseGate =
    !!profile &&
    (canOperateGateRole(profile) || memberOrganizationIds.length > 0);
    ...
    <header className="sticky top-0 z-40 border-b border-hairline bg-background/80 backdrop-blur-xl">
      <div className="container flex h-16 items-center gap-8">
        ...
        >
          ...
          <span className="font-mono text-[15px] font-medium tracking-[0.18em]">
            MISO
          </span>
        </Link>
