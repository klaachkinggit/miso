  ...
  return (
    <DialogPrimitive.Root
      ...
      onOpenChange={(nextOpen) => {
        setOpen(nextOpen);
        if (!nextOpen) setShowEscalate(false);
      }}
    >
      ...
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay className="fixed inset-0 z-40 bg-background/20" />
        <DialogPrimitive.Content
          ...
          className={cn(
            "fixed bottom-6 right-6 z-50 flex h-[34rem] max-h-[calc(100dvh-3rem)] w-[min(26rem,calc(100vw-3rem))] flex-col",
              ...
              >
                {messages.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    Ask about events, tickets, dates, or prices. I only know
