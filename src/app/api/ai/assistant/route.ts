...
import { clientIp, enforceRateLimit, rateLimitedResponseBody } from "@/lib/rate-limit";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await parseJsonBody(request, AssistantRequestSchema, "Invalid request.");
    ...
    const chunks = lastUser ? await retrieveOrgContext(org.id, lastUser.content) : [];
