...
export default async function SmartboardEventPage({
  ...
  const [compliance, { data: categories }, { data: controllerLinks }] = await Promise.all([
  ...
  const { data: controllerProfiles } = controllerIds.length
    ? await sb.from("profiles").select("*").in("id", controllerIds).returns<Profile[]>()
    : { data: [] as Profile[] };
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">{event.venue_name}, {event.city}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge variant={event.status === "published" ? "success" : "secondary"}>{event.status}</Badge>
            <Badge variant={compliance.live ? "success" : "warning"}>{compliance.live ? "Live" : "Sandbox"}</Badge>
          </div>
        </div>
      </div>
        ...
        <Card className="mb-6 rounded-lg border-amber-500/40 bg-amber-500/10">
          <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">Publishing is locked until legal and Stripe verification are complete.</span>
            </div>
            ...
            <Card className="glass h-fit rounded-lg">
              ...
              <CardContent className="grid gap-4 text-sm text-muted-foreground">
                <div className="flex items-center justify-between rounded-md border border-border/70 p-3">
                  ...
                  <Badge variant={(categories ?? []).length ? "success" : "warning"}>{categories?.length ?? 0}</Badge>
                </div>
                {event.status === "draft" ? (
                  <form action={publishOrganizerEvent}>
                    <input type="hidden" name="event_id" value={event.id} />
                    <Button type="submit" disabled={!compliance.live || !(categories ?? []).length} className="w-full">
                      Publish event
                    </Button>
                  </form>
                ) : (
          ...
          <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
            <div className="grid gap-4">
              {(categories ?? []).length ? (
                    ...
                    <CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
                      <div>
                        <div className="mb-2 flex flex-wrap items-center gap-2">
                          ...
                          <Badge variant="success">{category.supply} generated</Badge>
                        </div>
                ...
                <Card className="glass rounded-lg">
                  <CardContent className="p-5 text-sm text-muted-foreground">No ticket categories yet.</CardContent>
                </Card>
              )}
            </div>
            <Card className="glass h-fit rounded-lg">
                ...
                <form action={createOrganizerCategory} className="grid gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="grid gap-2">
                      ...
                      <select id="currency" name="currency" defaultValue="EUR" className="h-10 rounded-md border border-input bg-background/40 px-3 text-sm">
              ...
              {controllerProfiles?.length ? (
                ...
                <Card className="glass rounded-lg">
                  <CardContent className="p-5 text-sm text-muted-foreground">
                    {doorEligible ? "No door staff invited yet." : "Xpress Door is locked for this event."}
                  </CardContent>
                </Card>
              )}
            </div>
              ...
              <CardContent>
                <form action={inviteOrganizerController} className="grid gap-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input id="controller-email" name="email" type="email" required disabled={!doorEligible} />
                  </div>
                  ...
                  {!doorEligible ? (
                    <p className="text-xs text-muted-foreground">Publish the event and generate tickets before inviting door staff.</p>
                  ) : null}
                </form>
              </CardContent>
            </Card>
