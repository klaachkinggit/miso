...
import { ArrowRight, LockKeyhole, Mail, UserRound, type LucideIcon } from "lucide-react";
...
export function BuyerSignupForm({ error }: { error?: string }) {
  return (
    <form action={buyerSignupAction} className="grid gap-6">
      ...
      <Field icon={UserRound} label="Name" name="display_name" autoComplete="name" />
      ...
      <p className="text-center text-sm text-muted-foreground">
        Running events?{" "}
        <Link href="/signup/organizer" className="font-medium text-signal hover:underline">
          Organizer signup
        </Link>
      </p>
      <p className="text-center text-sm text-muted-foreground">
