...
export default async function OrganizerDashboardPage({
  ...
  const { organizations, activeOrganization } =
    await getActiveAdminOrganization(profile);
      ...
      <div className="container py-24">
        <div className="mx-auto max-w-xl text-center">
          ...
          <p className="mt-5 text-muted-foreground">
            Your ticketing workspace — events, team, payouts, and your public
            storefront all live inside it.
          </p>
  ...
  return (
    <div className="container py-10">
      <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-md text-muted-foreground">
            Sales, revenue, and door attendance across every event you run on
            MISO.
          </p>
        </div>
          ...
          {canCreateEvent ? (
            ...
            <Button asChild variant="outline">
              <Link href="/admin/settings">
                <CircleDollarSign className="h-4 w-4" /> Finish Stripe
                onboarding
              </Link>
            </Button>
          )}
              ...
              >
                <div>
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    ...
                    <Badge
                      variant={
                        event.status === "published" ? "signal" : "secondary"
                      }
                    >
                      {event.status}
                    </Badge>
      ...
      {activeOrganization ? (
        <CopilotPanel organizationId={activeOrganization.id} />
      ) : null}
    </div>
  );
}
