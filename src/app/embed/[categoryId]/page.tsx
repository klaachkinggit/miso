...
export default async function EmbedCategoryPage({
  ...
  const comingSoon =
    category.sale_starts_at != null && now < new Date(category.sale_starts_at).getTime();
  const salesEnded =
    category.sale_ends_at != null && now > new Date(category.sale_ends_at).getTime();
  const disabled = !category.sales_enabled || soldOut || comingSoon || salesEnded;
  ...
  const displayPrice =
    isClub && category.online_advance != null ? category.online_advance : category.price;
