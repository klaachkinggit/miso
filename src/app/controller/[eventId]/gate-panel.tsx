...
export function GatePanel({
      ...
      const res = await fetch("/api/controller/gates", {
        ...
        body: JSON.stringify({
          ...
          allowed_category_ids:
            scope === "selected" ? selectedCategoryIds : undefined,
        }),
      });
  ...
  async function closeGate() {
    if (!session) return;
    try {
      const res = await fetch(`/api/controller/gates/${session.id}/close`, {
        method: "POST",
      });
      const payload = (await res.json()) as GateSession & { error?: string };
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      const res = await fetch(`/api/controller/gates/${sessionId}`, {
        cache: "no-store",
      });
      if (!res.ok) return;
    ...
    <div className="rounded-md border border-hairline bg-ink-raised">
      ...
      <div className="grid gap-5 p-6">
        ...
        <Button
          type="button"
          onClick={openGate}
          disabled={creating || needsCategorySelection}
        >
          {creating ? (
