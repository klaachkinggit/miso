...
export async function startOrganizerOnboarding(formData: FormData) {
  ...
  if (!parsed.success) {
    fail(parsed.error.issues[0]?.message ?? "Complete the organizer questionnaire.");
  }
  ...
  try {
    ...
    fail(error instanceof Error ? error.message : "Organizer onboarding could not be started.");
  }
