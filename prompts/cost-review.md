...
1. **Skill bloat** — `ls .codex/skills/` and the active skill list. Every skill description loads each session. Prune anything unused for ~2 weeks.
2. **MCP server count** — inspect `.codex/config.toml`. Each server's tool list is in-context. Disable per-project servers you're not using.
