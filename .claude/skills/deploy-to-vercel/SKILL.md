...
Use the shell decision flow above and prefer the Vercel CLI when it is available.

For the no-auth fallback, run the deploy script from the skill's installed location:
```bash
bash .codex/skills/deploy-to-vercel/resources/deploy-codex.sh [path]
```
The path may vary depending on where the user installed the skill.

### Codex

Check if the CLI is available first, then fall back to the deploy script. Go to the **no-auth fallback — Codex sandbox** when auth or network access blocks the normal flow.
